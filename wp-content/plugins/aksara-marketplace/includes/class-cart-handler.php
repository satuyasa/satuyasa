...
class Aksara_Cart_Handler {
	...
	public static function init() {
		...
		// Dijalankan WooCommerce saat halaman cart dibuka DAN sekali lagi
		// sebelum checkout diproses — dua titik yang tepat untuk menangkap
		// item yang sudah basi sebelum uang berpindah.
		add_action( 'woocommerce_check_cart_items', array( __CLASS__, 'revalidate_cart_items' ) );
	}

	/**
	 * Hitung ulang harga & ketersediaan tiap item font yang ada di cart.
	 *
	 * Harga item font dihitung sekali saat ditambahkan lalu ikut tersimpan
	 * di session cart, dan apply_custom_price() cuma memutar ulang angka
	 * itu. Cart WooCommerce bisa bertahan berhari-hari, jadi tanpa
	 * pemeriksaan ulang:
	 *
	 *  - mengubah harga di admin TIDAK berpengaruh pada cart yang sudah
	 *    terlanjur terisi — pembeli checkout dengan harga lama;
	 *  - menghapus sebuah style atau jenis lisensi meninggalkan item yang
	 *    menunjuk ke data yang sudah tidak ada. Checkout tetap berhasil,
	 *    generate_tokens_for_order() membuat token untuk resource yang
	 *    hilang, dan pembeli yang sudah membayar hanya menerima
	 *    "File not found".
	 *
	 * validate_combo() sudah jadi satu-satunya tempat harga dihitung, jadi
	 * memanggilnya ulang di sini otomatis konsisten dengan jalur
	 * tambah-ke-keranjang.
	 */
	public static function revalidate_cart_items() {
		...
		foreach ( WC()->cart->get_cart() as $key => $cart_item ) {
			...
			$fresh = self::validate_combo(
				$cart_item['product_id'],
				$cart_item['aksara']['style_ids'],
				$cart_item['aksara']['license_id']
			);
			...
			if ( is_wp_error( $fresh ) ) {
				...
				wc_add_notice(
					sprintf(
						/* translators: 1: product name, 2: reason the item is no longer valid. */
						__( '"%1$s" was removed from your cart: %2$s', 'aksara-marketplace' ),
						$name,
						$fresh->get_error_message()
					),
					'error'
				);
				continue;
			...
			// Perbandingan float dengan toleransi, bukan !== : harga dibulatkan
			// ke 2 desimal di validate_combo() dan bolak-balik lewat session,
			// jadi kesamaan persis tidak bisa diandalkan.
			if ( abs( (float) $fresh['price'] - (float) $cart_item['aksara']['price'] ) > 0.009 ) {
				WC()->cart->cart_contents[ $key ]['aksara'] = $fresh;

				wc_add_notice(
					sprintf(
						/* translators: %s: product name. */
						__( 'The price of "%s" has changed since you added it to your cart. Your cart has been updated.', 'aksara-marketplace' ),
						$name
					),
					'notice'
				);
